...
<div class="comment">
What about the phylogenectic models?
</div>
...
<div class="comment">
Wow, that's some nifty work with the loop.

But, we could have used the piping operations we learned about to get deltaT. You're missing the importance of `pivot_wider()`.

```{r}
#dat3 to keep your naming convention
dat3 <- dat%>%
  group_by(subject,tod,mass)%>%
  na.omit()%>%
  filter(N>0.5*max(N))%>%
  summarize(m.temp=mean(Temp),n=n())%>%
  pivot_wider(names_from=tod,values_from = c(m.temp,n))%>%
  mutate(deltaT=abs(m.temp_day-m.temp_night))
...
<div class="comment">
Much of this paragraph above belongs in the methods.
...
<div class="comment">
These model tables are ugly to print and they don't have a caption. Try the `broom` package which tidys model output and use `kable`

```{r}
require(broom)
mass.lm%>%tidy%>%
  knitr::kable(caption="some informative caption")
```
...
<div class="comment">
Again, I like the effort here to combine data, but it woulda been much easier with the pipe.

Using my dat3 to start....


```{r}
human.dat <- dat3%>%
  ungroup()%>%
  summarize(deltaT=mean(deltaT,na.rm = TRUE),mass=mean(mass*1000,na.rm = TRUE),species="Homo sapians",Order="Primata")%>%
  dplyr::select(Order,species,mass,deltaT)
...
<div class="comment">
Again, we need a nice table with a caption. Use `kable`.
</div>
...
<div class="comment">
Well done using AIC in this context!
</div>
...
One of these factors can be phylogenetic information. When the mammals were grouped by order, anova analysis showed that order was a significant factor in predicting the Delta T to mass relationship since p = 5.224e-05 < 0.05. This means that there are more factors that effect Delta T in mammals than just mass. In further studies, factors such as climate and water supply of mammals could be investigated to further understand what predicts the level of heterothermy. 
